        ...
        if (len(p) == 17):
            if (isfloat(p[5]) and isfloat(p[11]) and isfloat(p[4]) and isfloat(p[15])):
                if (float(p[4])>= 2*60 and float(p[4])<= 60*60
                    ...
                    and float(p[5]) >= 1 and float(p[5]) <= 100
                    and float(p[15]) >= 3):
                    return p
...
#         exit(-1)
    ...
    load(sys.argv[1]) #sys.argv[1]
    testRDD = testDataFrame.rdd.map(tuple)
    taxilinesCorrected = testRDD.filter(correctRows)    
   
    #Task2 Linear Regression using Gradient Descent
      
    task2=taxilinesCorrected.map(lambda x: (float(x[11]),np.array(x[5])))
    task2=task2.map(lambda x:(x[0],np.append(x[1],1))) #yreal and x
    task2.cache()
    beta=np.ones(2)*0
    ...
    for i in range(10):
        ...
        gradientCost=task2.map(lambda x: (x[1], (x[0] - np.dot(x[1],beta)))).map(lambda x: (x[0]*x[1], x[1]**2 )).reduce(lambda x, y: (x[0] +y[0], x[1]+y[1]))
        
        cost= gradientCost[1]
        
        gradient=(-1/n)* gradientCost[0]
        print("iteration=",i, " Cost= ", cost,"learningrate =",learningRate)
        print("m= ",beta[0],"b= ",beta[1])
